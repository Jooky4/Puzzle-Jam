...
func _on_for_coins_buy_pressed(booster: Booster, count: Variant, cost: int) -> void:
	prints("buyed for coins", _price_info)

	if Player.coins >= cost:
		EventBus.coins_changed.emit(Player.coins - cost)
		Player.add_booster(booster.type, count)
		modal_close.emit()


func _on_for_ads_buy_pressed(booster: Booster, count: Variant, cost: int) -> void:
	prints("buyed for ads", _price_info)
	_price_info.count = count
